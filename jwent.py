...
def updateBoard(card_database, weather, p1_hand, p2_hand, p1_deck, p2_deck, p1_melee_line, p1_ranged_line, p1_siege_line, p2_melee_line, p2_ranged_line, p2_siege_line):
    biting_frost = False
    ...
    for card in p1_melee_line:             # look through played melee line cards
        ...
        for card_info in card_database:         # match the melee card to its database entry
            if card_info[:len(card)] == card and card != 'commander horn' and biting_frost == True: # weather condition
                points = 1
                if lookupCardAbility(card,card_database) == 'tight bond':
            ...
            elif card_info[:len(card)] == card and card != 'commander horn' and biting_frost == False:  # no weather condition
    ...
    for card in p1_ranged_line:             # look through played melee line cards
        points = 0
        for card_info in card_database:         # match the melee card to its database entry
            if card_info[:len(card)] == card and card != 'commander horn' and impenetrable_fog == True: # weather condition
                points = 1
                if lookupCardAbility(card,card_database) == 'tight bond':
            ...
            elif card_info[:len(card)] == card and card != 'commander horn' and impenetrable_fog == False:  # no weather condition
                points = lookupCardStrength(card, card_database)
                if lookupCardAbility(card,card_database) == 'tight bond':
                    points = p1_ranged_tight_bond_dict[card] * points  # add card strength
                if p1_ranged_horn == True:
    ...
    for card in p1_siege_line:             # look through played melee line cards
        points = 0
        for card_info in card_database:         # match the melee card to its database entry
            if card_info[:len(card)] == card and card != 'commander horn' and torrential_rain == True: # weather condition
                points = 1
                if lookupCardAbility(card,card_database) == 'tight bond':
            ...
            elif card_info[:len(card)] == card and card != 'commander horn' and torrential_rain == False:  # no weather condition
                points = lookupCardStrength(card, card_database)
                if lookupCardAbility(card,card_database) == 'tight bond':
                    points = p1_siege_tight_bond_dict[card] * points  # add card strength
                if p1_siege_horn == True:
        ...
        for card_info in card_database:  # match the melee card to its database entry
            if card_info[:len(card)] == card and card != 'commander horn' and biting_frost == True:  # weather condition
                ...
                if p2_melee_horn == True:
                 ...
                 :len(card)] == card and card != 'commander horn' and biting_frost == False:  # no weather condition
                points = lookupCardStrength(card, card_database)
                if lookupCardAbility(card, card_database) == 'tight bond':
                    points = p2_melee_tight_bond_dict[card] * points  # add card strength
                if p2_melee_horn == True:
    ...
    for card in p2_ranged_line:  # look through played melee line cards
        ...
        for card_info in card_database:  # match the melee card to its database entry
            if card_info[
               :len(card)] == card and card != 'commander horn' and impenetrable_fog == True:  # weather condition
                points = 1
                if lookupCardAbility(card, card_database) == 'tight bond':
                ...
                if p2_ranged_horn == True:
                 ...
                 :len(card)] == card and card != 'commander horn' and impenetrable_fog == False:  # no weather condition
                points = lookupCardStrength(card, card_database)
                if lookupCardAbility(card, card_database) == 'tight bond':
                    points = p2_ranged_tight_bond_dict[card] * points  # add card strength
                if p2_ranged_horn == True:
    ...
    for card in p2_siege_line:  # look through played melee line cards
        ...
        for card_info in card_database:  # match the melee card to its database entry
            if card_info[
               :len(card)] == card and card != 'commander horn' and torrential_rain == True:  # weather condition
                points = 1
                if lookupCardAbility(card, card_database) == 'tight bond':
                ...
                if p2_siege_horn == True:
                 ...
                 :len(card)] == card and card != 'commander horn' and torrential_rain == False:  # no weather condition
                points = lookupCardStrength(card, card_database)
                if lookupCardAbility(card, card_database) == 'tight bond':
                    points = p2_siege_tight_bond_dict[card] * points  # add card strength
                if p2_siege_horn == True:
    ...
    print('\n')
    return(p1_round_score, p2_round_score, weather, p1_hand, p2_hand, p1_deck, p2_deck, p1_melee_line, p1_ranged_line,
